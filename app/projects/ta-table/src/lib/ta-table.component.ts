...
loadDataFromServer(startIntial?: boolean, bypassCache = false): void {
    ...
    if (this.isAccountLedgerPage) {
        if (this.selectedAccountId) {
            // Use applyFilters which already has all the logic for account ledger
            this.applyFilters();
            return;
        } else {
    ...
    // 11. Pagination parameters (ALWAYS included)
    ...
    // Cache handling for selection popups
    const isSelectionPopup = !!this.options.rowSelectionEnabled;
    const cacheKey = isSelectionPopup
        ? `${apiUrl}|p${this.pageIndex}|s${this.pageSize}|q${this.options.globalSearch?.value || ''}`
        : null;
  ...
  onQueryParamsChange(params: NzTableQueryParams): void {
    ...
    // For Account Ledger page, use applyFilters() which preserves all filters
    if (this.isAccountLedgerPage && this.selectedAccountId) {
        this.applyFilters();
    } else {
        // For all other pages, loadDataFromServer() now preserves filters
        this.loadDataFromServer();
    }
}
  /**
